...
const ManageVendor = () => {
  ...
  useEffect(() => {
    ...
    fetch(`${import.meta.env.VITE_API_URL}/managevendors.php`)
      .then((response) => response.json())
      .then((data) => {
        if (data.status === "success") {
          // Filter the users to show only vendors
          const vendors = data.users.filter((user) => user.role === 'vendor');
          setUsers(vendors);
        } else {
  ...
  const handleDelete = (userId) => {
    if (window.confirm("Are you sure you want to delete this vendor?")) {
      fetch(`${import.meta.env.VITE_API_URL}/managevendors.php?id=${userId}`, {
        method: "DELETE",
      })
        .then((response) => response.json())
        .then((data) => {
